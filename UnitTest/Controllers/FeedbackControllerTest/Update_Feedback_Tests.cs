...
namespace UnitTest.Controllers.FeedbackControllerTest
{
    public class Update_Feedback_Tests
    {
        ...
        [Fact]
        public async Task Feedback_Update_ReturnsOkResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new FeedbackEditModel
            {
                FeedbackId = id,
            };

            var existingEntity = new FeedbackViewModel
            {
                FeedbackId = id,
            ...
            _mockService.Setup(service => service.UpdateAsync(id, model)).ReturnsAsync(new FeedbackViewModel { FeedbackId = id });
            ...
            var updatedModel = Assert.IsType<FeedbackViewModel>(okObjectResult.Value);
            Assert.NotNull(updatedModel);
            Assert.Equal(id, updatedModel.FeedbackId);
        }

        [Fact]
        public async Task Feedback_Update_ReturnsBadRequestResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new FeedbackEditModel
        ...
        [Fact]
        public async Task Feedback_Update_ReturnsBadRequestWhenIdMismatch()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new FeedbackEditModel
        ...
        [Fact]
        public async Task Feedback_Update_ReturnsInternalServerErrorResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new FeedbackEditModel
