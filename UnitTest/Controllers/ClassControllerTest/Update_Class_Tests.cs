...
{
    public class Update_Class_Tests
    {
        ...
        [Fact]
        public async Task Class_Update_ReturnsOkResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            ...
            var existingEntity = new ClassViewModel
            {
                ClassId = id,
            ...
            _mockService.Setup(service => service.UpdateAsync(id, model)).ReturnsAsync(new ClassViewModel { ClassId = id });
            ...
            var updatedModel = Assert.IsType<ClassViewModel>(okObjectResult.Value);
            Assert.NotNull(updatedModel);
            Assert.Equal(id, updatedModel.ClassId);
        }

        [Fact]
        public async Task Class_Update_ReturnsBadRequestResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new ClassEditModel
            ...
            _mockService.Setup(service => service.GetByIdAsync(id)).ReturnsAsync((ClassViewModel)null!);
        ...
        [Fact]
        public async Task Class_Update_ReturnsBadRequestWhenIdMismatch()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new ClassEditModel
        ...
        [Fact]
        public async Task Class_Update_ReturnsInternalServerErrorResult()
        {
            // Arrange
            var id = Guid.NewGuid();
            var model = new ClassEditModel
